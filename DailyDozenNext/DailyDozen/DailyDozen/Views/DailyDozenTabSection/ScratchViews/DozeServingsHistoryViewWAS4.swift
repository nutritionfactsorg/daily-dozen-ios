...
struct DozeServingsHistoryViewWAS4: View {
            ...
            GeometryReader { geometry in
                ScrollView(.horizontal, showsIndicators: true) {
                    Group {
                        ...
                        case .yearly:
                            yearlyChart
                                .frame(minWidth: CGFloat(processor.yearlyServings().count) * 40)
                        }
                    }
                    .padding(.vertical, 60)
    ...
    private var dailyChart: some View {
        ...
        .chartXAxis {
            AxisMarks(values: data.map { $0.date! }) { _ in
                AxisGridLine()
                AxisTick()
                //AxisValueLabel(format: xAxisFormat, anchor: .center)
                AxisValueLabel(format: xAxisFormat, centered: false)
                // Optional: Uncomment if labels need horizontal centering
                // .offset(x: 0)
            }
        }
        .chartYAxis {
    ...
    private func servingsAnnotation(servings: Int, year: Int? = nil) -> some View {
        Text("\(servings)")
            .font(.system(size: 10).bold()) // Reduced size for dense data
            .foregroundColor(.black)
            .padding(3)
            .background(servings > 0 ? Color.white.opacity(0.9) : Color.red.opacity(0.7))
            .clipShape(RoundedRectangle(cornerRadius: 4))
