...
struct SettingsReminderView: View {
    ...
    private func saveTime(_ time: Date) {
        ...
        UserDefaults.standard.set(dateWithOnlyTime.timeIntervalSinceReferenceDate, forKey: SettingsKeys.timeKeyPref)
    }
    ...
    private func saveSoundPref() {
        UserDefaults.standard.set(playNotifySound, forKey: SettingsKeys.reminderSoundPref)
    }
    ...
    private func loadTime() {
        if let interval = UserDefaults.standard.object(forKey: SettingsKeys.timeKeyPref) as? TimeInterval {
    ...
    private func setAlarm(for date: Date) {
        ...
        let trigger = UNCalendarNotificationTrigger(dateMatching: newComponents, repeats: true)  //TBDz check if this is false or true
