   ...
   // @State private var isGenerating1000 = false
    ...
    private func generateTestData(days: Int) async {
        ...
        do {
            ...
            print("•INFO•DB•GEN• SUCCESS GenerateHistoryView.generateTestData(\(days)) \(Date().datestampyyyyMMddHHmmssSSS)")
        } catch {
            await MainActor.run {
                errorMessage = "•ERROR•DB•GEN• GenerateHistoryView generateTestData \(error.localizedDescription)"
            }
            print("•INFO•DB•GEN• GenerateHistoryView.generateTestData(\(days)): \(error)")
        }
